...
3. `registerAlgorithm()` adds an algorithm (keyed by a stable `id`) to an in-memory registry. The built-in algorithms self-register as a side effect of importing `solver-core`'s entry point. They form a tower rather than a flat list: `level3-block-quotient` quotients Level 3 by its 400 mid-blocks and delegates to `level2-slice-reduction`, which fast-paths to `level2-block-quotient`, which delegates its macro solve to `level1-quotient`. Each rung projects onto the level below through the Menger self-similarity and lifts the answer back, so no rung reimplements the one under it.
